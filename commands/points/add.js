...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (!args[0]) return bot.commands.get("help").run(bot, message, ["add"]);

        const user = args[0];
        const amount = args[1];

        if (!user) return bot.commands.get("help").run(bot, message, ["add"]);
        if (!amount) return bot.commands.get("help").run(bot, message, ["add"]);
        ...
        if (body.success === false) return message.reply({ embeds: [bot.logger.err(`${user} does not exist on ROBLOX.`)] });
            ...
            message.reply({ embeds: [bot.logger.success(`Added ${amount} points to ${user}'s profile.`)] });
            ...
            if (logChannel) logChannel.send({ embeds: [bot.logger.info(`${message.author} added ${amount} points to ${rblxUsername}`)] });
            ...
            if ((currentXp + Number(amount)) >= requiredAmount && currentRankId < 24 && currentRankId > 0) {
